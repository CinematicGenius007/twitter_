...
## 2026-08-05 — Invite-only sign-up (P7)

Session scope: user asked to close public sign-up in favour of an invite-only flow, with invitations addressed to an email, a generated invite URL, updated auth pages, and the rest of the flow — leaving the build-vs-buy call (our own invite system vs whatever Clerk provides) to this session.

**The call: both, deliberately.** Clerk already has application invitations plus a "Restricted" sign-up mode, and its invitations carry email delivery and address verification for free — building that ourselves would have meant an email provider (Resend or similar) and our own token handling, i.e. hand-rolled auth, which `CLAUDE.md` forbids. But Clerk alone leaves us with no record of who invited whom, no allowance, and a rule that lives entirely in a dashboard toggle. So: Clerk owns the door and the mail, an `invites` table owns everything else, and `users.completeProfile` re-checks the rule server-side. That second check is the one that actually holds — it's the only code path that can create a `users` row, so the rule survives the dashboard being wrong.

**The binding is the email, not the link.** An invite row names one address; `completeProfile` matches it against `ctx.auth`'s Clerk-verified email. Codes are 128-bit and the public landing page masks the address, but even a fully leaked link is inert, because Clerk creates the account with the invited address and the gate matches on that. This is why the landing page can safely hand out the Clerk ticket URL at all.

Built:
- **Schema** — `invites` (email, code, inviterId, clerkInvitationId, clerkTicketUrl, status, createdAt, expiresAt, acceptedAt, acceptedUserId) + `users.invitedBy` / `users.inviteAllowance`.
- **`convex/invites.ts`** — split down the Convex actions/mutations seam: internal mutations do the transactional work (reserve a row *before* calling Clerk so two concurrent sends can't both pass the allowance check, discard it if Clerk refuses, revoke, begin-resend), actions do the Clerk Backend API calls. Queries: `mine` (owner-derived, no userId arg — same rule as bookmarks), `getByCode` (public, masked email), `myEligibility` (so `/complete-profile` can explain a refusal instead of bouncing a submitted form).
- **Gate** — `users.completeProfile` requires a pending unexpired invite for the caller's verified email, spends it in the same transaction as the profile insert (one letter can never admit two accounts), and records `invitedBy`. Founder exception fires only when the deployment has zero users. Gate runs before the rate limiter so a turned-away visitor gets the real reason, not "too many attempts".
- **Allowance** — 5 per member, refunded on revoke/lapse, not on accept. Rate limit `sendInvite` 5/hour, tighter than the app's other buckets because each send costs a real email against Clerk's 100/hour instance-wide limit.
- **UI** — `/invitations` ("Letters of Introduction"): send form, allowance counter, register of past letters with Awaiting reply / Taken up / Withdrawn / Lapsed, copy-link, send-again, withdraw. `/invitation/:code`: postmarked card naming the inviter and the masked address, accept button into Clerk's ticket. `RegisterPage` now shows an explanation instead of a form unless a `__clerk_ticket` is present (sticky, because Clerk's multi-step flow drops the query string on internal navigation — an early version would have ambushed people mid-sign-up with the refusal panel). `CompleteProfilePage` gained a refusal state with sign-off.

Snag worth remembering: `convex/_generated/api.d.ts` pulls every Convex module into the *frontend* tsc project, which is typed for the browser — so `process.env` in `convex/invites.ts` failed to compile even though Convex's runtime provides it. Fixed with a module-local `declare const process` rather than widening the app's `types` to include all of Node.

**Accidental production push.** `bunx convex dev --once`, run to typecheck the backend, picked up the production `CONVEX_DEPLOY_KEY` sitting in `apps/web/.env.local` and deployed to the prod deployment (`penny-post:production`) — adding the `invites` indexes and the new gate to live functions, and rewriting `VITE_CONVEX_URL` in `.env.local` to the prod URL. Schema change is additive so nothing was lost, but the invite gate is live in prod ahead of the manual setup below. Noted in `CLAUDE.md` §7 so the next session checks `CONVEX_DEPLOYMENT` first.

Left off: code complete, build + lint clean, `/register` and the `/invitation/:code` refusal states verified in-browser. **Not verified end-to-end**, because sending an invitation needs `CLERK_SECRET_KEY` and `APP_URL` set on the Convex deployment (credentials this session can't handle) plus the Clerk dashboard set to Restricted sign-up and an `email` claim in the `convex` JWT template. Those four steps and the end-to-end pass are the open items in `PLANNED.md` P7.
